...
export default function HelpRequestPage() {
  ...
  const [stats, setStats] = useState<HelpStats>({ activeExperts: 0, completedCount: 0, avgResponseHours: 0 });
  ...
  const filteredRequests = requests.filter((req) => {
    const typeLabel = HELP_PROBLEM_TYPE_LABELS[req.problemType] ?? req.problemType;
    const typeMatch = filter === "전체" || typeLabel === filter;
    ...
    <div className="px-4 py-6 max-w-lg mx-auto pb-24 min-h-screen bg-blob-violet">
      ...
      <div className="grid grid-cols-3 gap-2 mb-6">
        <div className="bg-card rounded-xl p-3 border border-border text-center">
          ...
          <div className="text-lg font-bold text-foreground">{stats.activeExperts}</div>
          <div className="text-[10px] text-muted-foreground">활동 전문가</div>
        </div>
        <div className="bg-card rounded-xl p-3 border border-border text-center">
          <CheckCircle className="w-5 h-5 text-green-500 mx-auto mb-1" />
          <div className="text-lg font-bold text-foreground">{stats.completedCount}</div>
          <div className="text-[10px] text-muted-foreground">해결 완료</div>
        </div>
        <div className="bg-card rounded-xl p-3 border border-border text-center">
          <Clock className="w-5 h-5 text-amber-500 mx-auto mb-1" />
          <div className="text-lg font-bold text-foreground">{stats.avgResponseHours}h</div>
      ...
      <div className="mb-4">
        ...
        <div className="flex gap-2 mb-3">
          {[
            ...
            <button
              key={status.value}
              onClick={() => setStatusFilter(status.value as "all" | "open" | "reviewing" | "closed")}
      ...
      <div className="space-y-3">
        {isLoading ? (
          ...
          filteredRequests.map((request) => {
            const statusColor = HELP_STATUS_COLORS[request.status] ?? "bg-gray-100 text-gray-600";
            const statusLabel = HELP_STATUS_LABELS[request.status] ?? "알 수 없음";
