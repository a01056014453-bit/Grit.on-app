...
🚨 최소 섹션 수 규칙:
- 짧은 곡(인벤션, 프렐류드, 소품 등)이라도 최소 3개 이상 섹션으로 나눌 것
- 예: A(주제 제시) → B(전개/전조) → A'(재현/종결)
- 1~2개 섹션만 출력하면 분석 실패로 간주
...
🚨 추천 연주: 최소 5명, 최대 7명. 1명만 출력하면 분석 실패.
   - 한국인 연주자 1명 이상 포함 (조성진, 임윤찬, 손열음 등)
...
🚨 4주 × 7일 = 28일. tasks 5개. 시간(분) 금지.
🚨 섹션이 3개 이하인 짧은 곡이라도 반드시 28일 전체를 채울 것.
   - 같은 섹션을 다른 관점에서 반복 (기초 → 기교 심화 → 표현 → 암보 → 템포 올리기 → 통주)
   - 각 일의 focus가 구체적으로 다른 과제를 다뤄야 함 (예: "A 섹션 성부 분리", "A 섹션 페달링", "B 섹션 암보")
🚨 1일만 출력하면 분석 실패. 반드시 4주 × 7일 = 28개 day 객체.
