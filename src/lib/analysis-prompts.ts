...
✅ 반드시 포함 (각 섹션마다):
- 해당 구간의 마디 번호 (예: "mm.1-8에서")
- 이 구간 고유의 기교적 도전이 무엇인지 (예: "만하임 로켓 상행 동기", "sforzando 대비")
- 구체적 연습 방법 3-4개 (예: "→ 상향 패시지 손가락 분산 훈련", "→ sf 직전 소리를 줄이고 갑자기 치솟게")
- 흔한 실수와 해결법 (예: "흔한 실수: 손목 경직 → 해결: 손목 이완을 의식적으로 유지")

예시:
구간 mm.1-8 (제1주제 도입):
→ 상향 패시지 정확한 손가락 분산 훈련
→ 빠른 업비트가 자연스러우면서도 추진력을 가질 수 있도록 박–박 사이 긴장 유지
→ 흔한 실수: 손목 경직 → 해결: 팔꿈치와 손목의 유연한 연결 훈련
...
[각 day의 tasks 규칙]
tasks는 반드시 4개로, 각각 구체적이어야 합니다:
1. 이 날 집중할 마디 구간 또는 기교 (예: "mm.1-8 만하임 로켓 상행 동기 반복 연습")
2. 양손/파트 따로 연습 → 구체적 목표 (예: "양손 따로 → sf 대비 표현 반복")
3. 표현/해석 과제 (예: "발전부 클라리넷-플루트 상상하며 대화하듯 연주")
4. 연결/통합 (예: "제시부와 발전부 연결, 템포 일정 유지")
